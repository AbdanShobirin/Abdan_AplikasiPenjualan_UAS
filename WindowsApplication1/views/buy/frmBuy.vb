Public Class frmBuy
    ...
    Private Sub frmBuy_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub dgvItems_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles dgvItems.CellEndEdit
        ...
        ElseIf e.ColumnIndex = dgvItems.Columns("qtyBuy").Index Then
            HitungSubtotal(e.RowIndex)
        ElseIf e.ColumnIndex = dgvItems.Columns("priceBuy").Index Then
            HitungSubtotal(e.RowIndex)
        End If
    End Sub
    Private Sub HitungSubtotal(rowIndex As Integer)
        Dim qty = CDec(dgvItems.Rows(rowIndex).Cells("qtyBuy").Value)
        Dim harga = CDec(dgvItems.Rows(rowIndex).Cells("priceBuy").Value)
    ...
    Private Sub IsiDetailBarang(rowIndex As Integer, kode As String)
        ...
        dgvItems.Rows(rowIndex).Cells("priceBuy").Value = CDec(rowBarang.salesPrice)
        dgvItems.Rows(rowIndex).Cells("unit").Value = rowBarang.unit

        ' Default jumlah = 1 jika kosong
        If dgvItems.Rows(rowIndex).Cells("qtyBuy").Value Is Nothing OrElse dgvItems.Rows(rowIndex).Cells("qtyBuy").Value = "" Then
            dgvItems.Rows(rowIndex).Cells("qtyBuy").Value = 1
        End If
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Dim buy As New buyModel With {
            .idTrans = txtKode.Text,
            .buyDate = DateTime.Now,
            .details = New List(Of buyDetailModel)
        }
        ...
        For Each row As DataGridViewRow In dgvItems.Rows
            ...
            If kode <> "" Then
                ...
                buy.details.Add(New buyDetailModel With {
